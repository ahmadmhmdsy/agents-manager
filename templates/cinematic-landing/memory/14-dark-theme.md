...
> **Light-mode contrast footnote (Fix 1, v0.14.0):** Light-mode body text uses
> `--ink:#3A2E1F` (PASS 12.6:1) and `--ink-soft:#5C4D3A` (PASS 7.4:1) on
> `--paper:#FBF6EE`. Index markers (`.idx`) use `--gold-bright:#CC9A4A` on
> light-mode `#ritual` — never `--gold-deep` on light, which fails AA body at 2.35:1
> on the literal ambient and 2.33–2.80:1 on the actual on-screen composite
> (photo + rgba overlay at the `.idx` position; see §3 P5 audit).
...
- K2 `#ritual .idx` (top stop = `--gold-deep`): light 2.43:1 vs literal `#3B2A1B` ambient — P5 reviewed 2026-07-03; verdict: --gold-bright wins on .idx; see §3 for measurement methodology; dark 5.63:1 ✓
