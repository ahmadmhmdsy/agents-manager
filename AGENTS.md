...
Downstream projects can customize any of the 10 SKILL.md files (master + 9 specialists) without losing changes on `agents-manager update`. Convention: create `agents_manager/<role>/SKILL.local.md` alongside `SKILL.md` and put your customizations there. The agent reads both at boot; `.local.md` is the addendum, `SKILL.md` is the upstream default. The existing SKIP-if-exists behavior in `bin/agents-manager` preserves `.local.md` across updates. v0.24.0 only supports ADD via `.local.md`; REMOVE coming in a future release.
