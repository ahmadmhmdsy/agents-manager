---
name: self-reflective-prompt
description: Activate a structured self-reflection pass on the master agent behavior, accumulated session patterns, or system prompt drift. Use when the user requests reflection, repeated pipeline failures occur, drift is detected, or before a major handoff. Pattern borrowed from SELF_REFLECTIVE_PROMPT_IMPROVEMENT_AGENT.md. Do not use for normal task execution or routine progress checks.
---
