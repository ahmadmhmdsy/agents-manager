...
description: Apply a multi-dimensional code verification and validation rubric during review. Use for severity classification (CRITICAL/HIGH/MEDIUM/LOW), per-task verdict impact, and review scope definition. Pattern borrowed from verification-validation-system-prompt.md. Do not use as a stand-alone task; this rubric layers onto an existing review session.
---
...
## DIMENSION 8: PROJECT-SPECIFIC INVARIANTS (D9.5 Smart Sync)

These are non-negotiable invariants for the إلكترونك-سيتي (Electronik City) Android project after the D9.5 Smart Sync rollout. Treat any violation as **CRITICAL** — they map directly to data-leak or cost-explosion blast radius in production.

### 1. **Admin-isolation invariant** (D27)

- **Rule:** No code outside `com.example.data.admin.*` and `com.example.ui.screens.admin.*` may import any of:
  - `com.example.data.admin.AdminSettingsRepository`
  - `com.example.data.admin.AdminCostRepository`
  - `com.example.data.admin.AdminPresenceRepository`
  - `com.example.data.sync.AdminSyncCoordinator`
- **Why:** The admin split is part of a 3-layer defense (Firestore rules + package split + this check). A non-admin ViewModel that reaches into an admin repository can read/write data the user is not entitled to; the rules will reject the call but the app will have leaked intent.
- **How to check:** Run detekt (P10.1) — `ForbiddenImport` is enabled in `config/detekt/detekt.yml`. Also grep for the four FQNs in non-admin Kotlin files.
- **Severity:** CRITICAL.

### 2. **Usage-counter increment invariant** (D9.5 V.2)

- **Rule:** Every call to a `FirestoreRemoteDatabaseService` method that performs a Firestore read/write/delete must invoke `usageCounter.incrementRead()` / `incrementWrite(count)` / `incrementDelete()` inside the same `try { ... }` block (or before the first `await()` that hits Firestore).
- **Why:** The T5 Cost Dashboard is the only thing standing between us and a runaway Firestore bill. If a new method ships without incrementing, the dashboard will under-count and the admin will not know to throttle.
- **How to check:** Grep every `await()` call in `FirestoreRemoteDatabaseService.kt` and confirm it is paired with an `increment*` call on the same line/block. The T2 background sync path (in `SyncCoordinator`) must also increment, even though it does not call `FirestoreRemoteDatabaseService` directly.
- **Severity:** CRITICAL.

### 3. **Presence-write PII invariant** (D9.5 V.7)

- **Rule:** The `writePresence` body may only write: `sessionId` (UUID), `currentProductId` (nullable String), `lastSeen` (long), `appVersion` (String). It must NOT write `uid`, `email`, `phone`, `username`, `name`, `location`, or any other `UserProfile` field.
- **Why:** Firestore rules allow any auth user to write to any `presence/{sessionId}` document. If we accidentally PII-leak into the body, the privacy model breaks — and we cannot redact historical docs without nuking the collection.
- **How to check:** Inspect `FirestoreRemoteDatabaseService.writePresence(...)` body. Grep for `auth.uid` — should be absent inside the function body (the `ensureSignedIn()` helper is the only place that references `auth.currentUser`, and only to gate the call, not to populate the doc).
- **Severity:** CRITICAL.

### 4. **Force-refresh admin claim invariant** (D9.5 V.5)

- **Rule:** Any code path that depends on the `role == "admin"` custom claim must force-refresh the ID token via `auth.currentUser?.getToken(true)?.await()` *before* checking the claim, on every call site — not cache the result.
- **Why:** Cached ID tokens are valid for ~1 hour. A freshly-promoted admin would be locked out for up to an hour if we read the stale claim. This is the exact failure mode documented in the d9.5 design doc.
- **How to check:** Inspect `FirestoreRemoteDatabaseService.requireAdmin` — confirm the first line of the claim check calls `getToken(true)`. The check should appear on every entry point, not a shared helper.
- **Severity:** HIGH.

### 5. **Price-rendering composable invariant** (D9.5 P10.3)

- **Rule:** Every composable that renders a `Product.price`, `Product.originalPrice`, or `Order.totalPrice` must subscribe to the underlying Room flow via `collectAsState` (or `collectAsStateWithLifecycle`). A `remember { mutableStateOf(product.price) }` pattern is forbidden — the price will go stale after an admin edit and the user will see a phantom price until the next recomposition triggers a re-read.
- **Why:** The admin can update prices, discounts, and stock in real time (admin panel → Firestore → T1 sync → Room). If a price-rendering composable snapshots the value at composition time, the user sees the old price until they navigate away and back.
- **How to check:** Grep `grep -rn "mutableStateOf.*\\.price\\|mutableStateOf.*\\.originalPrice\\|mutableStateOf.*\\.totalPrice" app/src/main/java/com/example/ui/` — should return 0 matches. The only `mutableStateOf` allowed in price contexts is the local UI state (e.g., quantity stepper in cart) — never the source price.
- **Severity:** HIGH.
